...
use crate::ai::{generate_email_tags, has_available_provider};
use crate::config::get_config;
use crate::mail::{
    add_tag_to_email, create_tag, delete_email_from_cache, get_accounts,
    get_email_for_tagging, get_emails_for_auto_tagging, log_op, sync_emails_with_session, Account,
};
...
async fn process_sync(account_name: &str, mailbox: &str) {
    ...
    match result {
        Ok(Ok(sync_result)) => {
            ...
            process_auto_tagging(account_name, mailbox).await;
        }
        Ok(Err(e)) => {
...
/// Process auto-tagging for emails after sync completes.
/// Runs in a background task, doesn't block sync completion.
async fn process_auto_tagging(account_name: &str, mailbox: &str) {
    // 1. Check if auto-tagging is enabled in config
    let config = match get_config() {
        Ok(c) => c,
        Err(e) => {
            eprintln!("[auto-tag] Failed to load config: {}", e);
            return;
        }
    };

    let auto_tag_config = match config.auto_tagging {
        Some(c) if c.enabled => c,
        _ => {
            // Auto-tagging disabled or not configured
            return;
        }
    };

    // 2. Check if any AI provider is available
    if !has_available_provider() {
        eprintln!("[auto-tag] No AI provider available, skipping auto-tagging");
        return;
    }

    // 3. Get emails that need tagging
    let max_emails = auto_tag_config.max_emails_per_sync;
    let uids = match get_emails_for_auto_tagging(account_name, mailbox, max_emails) {
        Ok(u) => u,
        Err(e) => {
            eprintln!("[auto-tag] Failed to get emails for tagging: {}", e);
            return;
        }
    };
    ...
    eprintln!(
        "[auto-tag] Processing {} emails for {}/{}",
        uids.len(),
        account_name,
        mailbox
    );

    let rate_limit_ms = auto_tag_config.rate_limit_ms;
    let account = account_name.to_string();
    let mbox = mailbox.to_string();

    // 4. Spawn background task for tagging (non-blocking)
    tokio::spawn(async move {
        for (idx, uid) in uids.iter().enumerate() {
            // Rate limit: delay between calls (except first)
            if idx > 0 {
                tokio::time::sleep(Duration::from_millis(rate_limit_ms)).await;
            }

            // Get email data
            let email_data = match tokio::task::spawn_blocking({
                let account = account.clone();
                let mbox = mbox.clone();
                let uid = *uid;
                move || {
                    get_email_for_tagging(&account, &mbox, uid)
            ...
            .await
            {
                ...
                Ok(Ok(None)) => {
                    eprintln!("[auto-tag] Email {} not found or no body", uid);
                    continue;
                }
                Ok(Err(e)) => {
                    eprintln!("[auto-tag] Failed to get email {}: {}", uid, e);
                    continue;
                }
                Err(e) => {
                    eprintln!("[auto-tag] spawn_blocking error: {}", e);
                    continue;
                }
            };

            let (from, subject, body_html) = email_data;

            // Convert HTML to plain text for AI
            let body_text = html_to_text(&body_html);

            // Generate tags via AI
            let tags = match tokio::task::spawn_blocking({
                let from = from.clone();
                let subject = subject.clone();
                let body_text = body_text.clone();
                move || generate_email_tags(&from, &subject, &body_text)
            })
            .await
            {
                Ok(Ok(tags)) => tags,
                Ok(Err(e)) => {
                    eprintln!("[auto-tag] AI tag generation failed for {}: {}", uid, e);
                    continue;
                }
                Err(e) => {
                    eprintln!("[auto-tag] spawn_blocking error: {}", e);
            ...
            if tags.is_empty() {
                eprintln!("[auto-tag] No tags generated for {}", uid);
                continue;
            }

            // Apply tags to email
            let account_ref = account.clone();
            let mbox_ref = mbox.clone();
            let uid_val = *uid;

            for tag_name in &tags {
                // Create or get tag (use purple as default color for auto-generated tags)
                let tag = match create_tag(tag_name, "#7C3AED") {
                    Ok(t) => t,
                    Err(e) => {
                        eprintln!("[auto-tag] Failed to create tag '{}': {}", tag_name, e);
                        continue;
                    }
                };

                // Add tag to email with auto_generated=true
                if let Err(e) = add_tag_to_email(&account_ref, &mbox_ref, uid_val, tag.id, true) {
                    eprintln!("[auto-tag] Failed to add tag '{}' to {}: {}", tag_name, uid_val, e);
                }
            }

            eprintln!(
                "[auto-tag] Tagged email {} with {:?} ({}/{})",
                uid,
                tags,
                idx + 1,
                uids.len()
        ...
        eprintln!("[auto-tag] Completed auto-tagging for {}/{}", account, mbox);
    });
}
