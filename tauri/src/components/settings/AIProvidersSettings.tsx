...
export function AIProvidersSettings({
  ...
  const [sdk, setSdk] = useState<string>("openai");
  const [customHeaders, setCustomHeaders] = useState<string>("");
  const [testingProvider, setTestingProvider] = useState<number | "new" | null>(
    null,
  );
  ...
  const saveProvider = async () => {
    ...
    const provider: AIProvider = {
      ...
      base_url: baseUrl,
      api_key: apiKey,
      sdk: providerType === "api" && sdk !== "openai" ? sdk : undefined,
  ...
  const testProvider = async (
    ...
    testApiKey: string,
    testSdk?: string,
    testCustomHeaders?: Record<string, string>,
  ) => {
    ...
    try {
      ...
      if (type === "cli") {
        // CLI providers must be tested via Rust backend
        result = await invoke<TestResult>("test_ai_provider", {
        ...
        // API providers can be tested directly via JS SDK
        const providerConfig: AIProviderConfig = {
          ...
          baseUrl: testBaseUrl,
          apiKey: testApiKey,
          sdk: (testSdk as AIProviderConfig["sdk"]) || "openai",
          customHeaders: testCustomHeaders,
        };
        const response = await testApiProvider(providerConfig);
        result = {
        ...
        <CardContent className="space-y-4">
          ...
          >
            ...
            <DialogContent className="max-h-[85vh] flex flex-col sm:max-w-xl">
              ...
              <div className="space-y-4 py-4 overflow-y-auto flex-1 -mr-6 pr-6">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="cli">
                        {t("settings.ai.typeCli")}
                      </SelectItem>
                      <SelectItem value="api">
                        {t("settings.ai.typeApi")}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                {providerType === "api" && (
                  <>
                    <div className="grid gap-2">
                      <Label htmlFor="provider_base_url">
                    ...
                    <div className="grid gap-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Select the SDK matching your provider
                      </p>
                    </div>

                    <div className="grid gap-2">
                      <Label htmlFor="provider_custom_headers">
                        {t("settings.ai.customHeaders")}
                      </Label>
                      <Textarea
                        id="provider_custom_headers"
                        placeholder={
                        ...
                        disabled={isReadOnly}
                        rows={4}
                        className="font-mono text-sm max-h-32 resize-none p-3"
                      />
                      <p className="text-xs text-muted-foreground">
                        JSON format. Some providers require custom HTTP headers.
                      </p>
                    </div>
                  </>
                )}
